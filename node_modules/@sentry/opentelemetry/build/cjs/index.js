  ...
  /** @inheritDoc */
   shouldSample(
          ...
          if (ignoreSpans?.length) {
            ...
            if (
              core.shouldIgnoreSpan(
                { description: inferredChildName, op: spanAttributes[core.SEMANTIC_ATTRIBUTE_SENTRY_OP] ?? childOp },
                ignoreSpans,
              )
            ) {
    ...
    if (
      ...
      ignoreSpans?.length &&
      core.shouldIgnoreSpan(
        { description: inferredSpanName, op: mergedAttributes[core.SEMANTIC_ATTRIBUTE_SENTRY_OP] ?? op },
        ignoreSpans,
      )
    ) {
      this._client.recordDroppedEvent('ignored', 'span');
